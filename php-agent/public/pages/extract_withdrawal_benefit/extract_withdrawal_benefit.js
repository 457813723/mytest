(function (G){
    ...
    G.ajax({
        ...
        let { history_income, usable_income, used_income} = data.content;

        let html = '';
        html += `<div class="aui-flex aui-flex-mar b-line">`;
        html += `<div class="aui-flex-head">累计推广收益（元）：</div>`;
        html += `<div class="aui-flex-box" id="sanjifanli">${history_income}</div>`;
        html += `</div>`;
        html += `<div class="aui-flex aui-flex-mar b-line">`;
        html += `<div class="aui-flex-head">今日可提 （累计余额 - 已提收益）</div>`;
        html += `<div class="aui-flex-box" id="canCarryToday"> ${usable_income} </div>`;
        html += `</div>`;
        html += `<div class="aui-flex aui-flex-mar b-line">`;
        html += `<div class="aui-flex-head">已提收益：</div>`;
        html += `<div class="aui-flex-box" id="yitishouyi">${used_income}</div>`;
        html += `</div>`;
