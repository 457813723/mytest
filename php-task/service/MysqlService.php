...
class MysqlService {
    ...
    public function __construct(array $conf) {
        ...
        $this->_host = config('my_host');
        $this->_port = config('my_port');
        $this->_user = config('my_user');
        $this->_pass = config('my_pass');
    ...
    protected function _doQuery($sql='') {
        ...
        try{
            ...
            {
                echo "MySQL server has gone away, try to reconnection...";
                $this->_connect();
            }
...
//        $pdostmt = self::$_dbh->prepare($this->_sql);
    ...
    protected function _doExec($sql='') {
        $this->_sql = $sql;
        try{
            $res = self::$_dbh->exec($this->_sql);//prepare或者query 返回一个PDOStatement
        }
        catch(PDOException $e){
            if((int)$e->errorInfo[1]== 2006 &&$e->errorInfo[2] == 'MySQL server has gone away')
            {
